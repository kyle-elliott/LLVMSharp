...
public unsafe partial struct LLVMMetadataRef(IntPtr handle) : IEquatable<LLVMMetadataRef>
{
    ...
    public readonly bool IsVariable => Kind switch {
        ...
        LLVMMetadataKind.LLVMDIGlobalVariableExpressionMetadataKind => true,
        _ => false,
    };

    public readonly LLVMMetadataKind Kind => Handle == default
        ? (LLVMMetadataKind)(-1) // 0 is a valid kind (LLVMMetadataKind.LLVMDILocationMetadataKind), so we use -1 to represent a null metadata reference
